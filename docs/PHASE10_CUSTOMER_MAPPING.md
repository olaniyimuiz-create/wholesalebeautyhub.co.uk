...
| `billing_company` | **`MailingAddressInput.company` — address level, NOT customer level** | **Corrected 2026-08-19.** `CustomerInput` has no `company` field (verified). A customer with no address therefore cannot carry a company at all. |
...
| `date_registered` | `metafield custom.woo_registered_at` (**proposed**) | Informational only. `Customer.createdAt` is server-set and **read-only** — a metafield is the only possible destination, not a stylistic preference. Under the architecture ratified 2026-08-21 this field rides inline in `CustomerInput.metafields` at **no additional API call**, so retaining or dropping it is a pure cost/value question with no architectural consequence (Decision #8 in `PHASE10_DECISION_MATRIX.md`). |
...
| WooCommerce | Shopify | Notes |
|---|---|---|
| `billing_address_1`/`_2`, `billing_city`, `billing_state`, `billing_postcode`, `billing_country` (usermeta, or `wp_wc_customer_lookup`'s own city/state/postcode/country columns, or guest order-billing fallback) | **`customerAddressCreate(customerId:, address: MailingAddressInput!, setAsDefault:)` — a SEPARATE second call** | **Corrected 2026-08-19.** This previously read `addresses: [CustomerAddressInput]` on `CustomerInput`. **That field does not exist in API 2026-07** — verified by live introspection; `CustomerInput` declares no `addresses`, and the type is `MailingAddressInput`, not `CustomerAddressInput`. Addresses require a second mutation per address, after the customer exists. 4,729 of 12,096 IMPORT customers have a billing address; 7,351 have no address at all (61%) — Shopify does not require one. |
| `shipping_address_1`/`_2`, `shipping_city`, `shipping_state`, `shipping_postcode`, `shipping_country` (usermeta) | **NOT CURRENTLY MAPPED** | Real gap: `database_parser.py`'s `is_customer_meta_key()` already captures these into `usermeta` (its whitelist includes any `shipping_`-prefixed key), but `build_customers()` never reads them out — `migration/data/customers.json` and the pre-built `shopify_customers_import.csv` both carry **billing only**. 1,209 of 12,096 customers (10%) have a real shipping address that is currently discarded, not migrated. Shopify supports a second address via another `customerAddressCreate` call (**not** a second entry in a non-existent `addresses` input) — a genuine, fixable gap, not a platform limitation. Note Shopify has **no billing/shipping distinction** on customer addresses: both land in one undifferentiated list, and only `setAsDefault` separates them. 16 of these customers have a shipping address but no billing address, so shipping would be their only address. No decision has been made yet on whether to add it. |
| `billing_state` / `Default Address Province Code` | `provinceCode` — **omitted entirely for GB** | `MailingAddressInput` has no free-text `province`, only `provinceCode`. 2,483 GB customers carry a county name ("Surrey", "West Midlands") in `billing_state`, which is not a province code and would be rejected or silently dropped. `phase10_import_runtime.build_address_input()` sends `provinceCode` only for countries in `PROVINCE_CODE_COUNTRIES` (GB deliberately absent) and flags the drop rather than discarding it silently. |
| `billing_country` | `countryCode` (**`CountryCode` enum, not a string**) | 245 valid codes. 16 IMPORT customers have an address with no country and cannot have one constructed — see the open business decision. `ZZ` ("Unknown Region") is enum-valid but refused as undeliverable. |
