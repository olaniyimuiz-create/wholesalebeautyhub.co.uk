...
**Reconciliation performed for real (2026-08-10)**:
`reports/phase9_test_import_reconciliation.csv`, generated by
`migration/scripts/phase9_test_reconcile.py` — a fresh, independent,
read-only query against the live Shopify store for each of the 9 test
products, compared field-by-field to `products.json`. Not the same file
as this template (kept as schema-only) or `reports/phase9_reconciliation.csv`
(Phase 9 dry run, source-vs-local-payload only, never contacted Shopify).

Result: 134 field-level comparisons, 131 matched, 3 real mismatches — not
hidden, not rounded up to 100%. All 3 trace to the two findings in
`docs/RISK_REGISTER.md` #33/#34 and GitHub issue #41.
