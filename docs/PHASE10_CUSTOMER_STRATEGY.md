...
| Address support | **`customerAddressCreate` — a separate mutation per address**, using `MailingAddressInput`. **Corrected 2026-08-19**: this previously read `addresses: [CustomerAddressInput]`, a field that does not exist on `CustomerInput` in API 2026-07 (verified by live introspection). Multiple addresses per customer remain possible, at one extra call each. Shopify draws no billing/shipping distinction — one undifferentiated list, ordered by `setAsDefault` | One default address per row (matches the pre-built CSV's current shape) |
